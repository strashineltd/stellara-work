...
默认情况下，API 密钥、会话、文件与配置**只留在本机**；网络请求仅发往你配置的模型端点。可选的云账号登录（设置 → 账号）会将身份与登录材料发送到腾讯云 CloudBase；若需完全离线工作流请勿启用。已连接的 opencode 服务器与 MCP HTTP 服务器也会收到你指向它们的流量。
...
| | 功能 | 说明 |
|---|---|---|
| 🔒 | **本地隐私优先** | API 密钥由系统钥匙串（macOS）/ DPAPI（Windows）加密；默认数据留本机（启用云账号或连接远端服务器除外） |
