...
- **界面重设计**：按参考图重写应用外壳 — 新顶栏（前进/后退导航、侧栏与工作区面板切换）、新首页（问候语 + 「探索并理解代码 / 构建新功能 / 审查代码 / 修复问题」能力卡片 + 浮动输入卡）、会话视图头部与设置面板皮肤
- **Pull Request 占位页**：「Pull Request 将在后续版本推出」空状态页，含返回首页
- **服务器连接**：设置 → 服务器支持添加/编辑/测试/设为默认与加密凭据
- **服务器会话**：顶栏执行端切换，支持创建会话、流式对话、审批、模型与 Agent 切换
- **侧栏服务器分组**：会话按服务器归类，离线显示徽标并支持重连
- **已安排（定时调度）**：支持一次性 / 间隔 / Cron 三种调度；启动时错过补偿（补记一条 `missed` 记录并立即补跑一次）；任务、执行记录与执行会话按本地身份隔离
- **调度执行**：本地任务以无 UI 会话复用 Agent 循环，服务器任务走已连接的 opencode 会话；完成 / 失败发系统通知；危险工具默认拒绝（失败关闭），`allow_dangerous` 仅作 UI 风险确认、不改变运行时拒绝语义
- **托盘驻留**：关闭窗口隐藏到托盘、调度照常运行；`Cmd+Q` / 托盘「退出」完全退出；设置新增「关闭窗口后保持后台运行（调度继续）」（默认开）
- **「已安排」页面**：任务列表（执行端徽标 / 下次运行 / 最近状态 / 启停 / 立即运行 / 编辑 / 删除）、新建与编辑对话框（一次性时间 / 每隔 N 分钟·小时 / Cron 表达式 + 常用预设）、执行历史（最近 20 条，含会话跳转与错误信息）；运行中的任务可一键停止（本地中断 Agent 循环 / 服务器发送 /abort）
...
- 侧栏信息架构：主导航改为「新对话 / Pull Request / 已安排」；项目分组嵌套会话，「最近」收纳未分组会话；底部工具区改为记忆 / 文件 / 设置；原「项目」独立页面取消，项目管理收敛到分组头；移除插件市场入口
